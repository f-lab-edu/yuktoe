...
import 'package:yuktoe/presentation/record_detail/widgets/common/left_right_card.dart';
...
class PumpingDetailView extends StatelessWidget {
  final PumpingDetail detail;

  const PumpingDetailView({super.key, required this.detail});

  @override
  Widget build(BuildContext context) {
    final vm = context.read<RecordDetailViewModel>();
    final left = detail.leftAmountMl ?? 0;
    final right = detail.rightAmountMl ?? 0;
          ...
          child: LeftRightCard(
            leftValue: EditableChip(
              ...
              onTap: () async {
                ...
                if (picked != null) {
                  vm.stageDetail(detail.copyWith(leftAmountMl: picked));
                }
              },
            ),
            rightValue: EditableChip(
              text: '${right}ml',
              onTap: () async {
                final picked = await showNumberInputModal(
                  context,
                ...
                if (picked != null) {
                  vm.stageDetail(detail.copyWith(rightAmountMl: picked));
                }
              },
            ),
            totalText: '총 ${left + right}ml',
          ),
        ),
      ],
    );
